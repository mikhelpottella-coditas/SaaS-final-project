...
public class CityManagementController {
    ...
    @Operation(summary = "get all the areas under my district ")
    @GetMapping("/areas/city/{cityId}")
    public ResponseEntity<List<AreaResponseDto>> getAllAreas(@PathVariable Long cityId,
        ...
        return ResponseEntity.ok(areaService.getAll(cityId,page,size,sortBy,ascending,search));
    }


    @Operation(summary = "get the areas under my district by id ")
    @GetMapping("/areas/{areaId}")
    public ResponseEntity<AreaResponseDto> getAllAreas(@PathVariable Long areaId) {
