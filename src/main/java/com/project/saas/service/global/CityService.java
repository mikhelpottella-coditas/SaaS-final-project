...
public class CityService {
    ...
    public List<CityResponseDto> getAllCities(int page, int size, String sortBy, boolean ascending, String search) {
        ...
        citiesList.forEach(cities -> {
            ...
            List<Long> areaIds = cities.getAreaList() == null ? null : cities.getAreaList().stream().map(Area::getId).toList();
    ...
    public String deleteCityById(Long id) {
        ...
        try {
            cityRepo.deleteById(cities.getId());
        } catch (Exception e) {
            throw new CustomException(HttpStatus.BAD_REQUEST, " city is not deleted, since they are dependent things which are to be deleted first ");
        }

        log.info("the city deleted successfully");
